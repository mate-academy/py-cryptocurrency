...
def test_rate_is_more_than_5_higher(
        mocked_func: mocked_func) -> None:
    mocked_func.return_value = 1.06
    assert cryptocurrency_action(1) == "Buy more cryptocurrency"


def test_rate_is_more_than_5_lower(
        mocked_func: mocked_func) -> None:
    mocked_func.return_value = 0.94
    assert cryptocurrency_action(1) == "Sell all your cryptocurrency"


def test_difference_is_not_that_much(
        mocked_func: mocked_func) -> None:
    mocked_func.return_value = 1.05
    assert cryptocurrency_action(1) == "Do nothing"


def test_difference_is_not_that_much_one_more(
        mocked_func: mocked_func) -> None:
