...
def test_get_exchange_rate_prediction(
    ...
    with mock.patch("random.choice", return_value=mock_choice), \
        mock.patch("random.random", return_value=mock_random):
        result = get_exchange_rate_prediction(exchange_rate)

        assert result == expected_result, f"Expected {expected_result}, but it turned out {result}"
...
def test_cryptocurrency_action(
    ...
    with mock.patch("app.main.get_exchange_rate_prediction", return_value=prediction_rate):
