...
def test_cryptocurrency_action_with_better_rate(mock_prediction) -> None:
    mock_prediction.return_value = 1
    assert cryptocurrency_action(1.5) == "Sell all your cryptocurrency"


def test_cryptocurrency_action_with_bad_rate(mock_prediction) -> None:
    mock_prediction.return_value = 1.5
    assert cryptocurrency_action(1) == "Buy more cryptocurrency"


def test_cryptocurrency_action_first_exception(mock_prediction) -> None:
    mock_prediction.return_value = 0.95
    assert cryptocurrency_action(1) == "Do nothing"


def test_cryptocurrency_action_second_exception(mock_prediction) -> None:
