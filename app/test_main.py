...
class TestCryptocurrencyAction(TestCase):
    ...
    def test_buy_more_when_prediction_high(
        self, mock_prediction: Mock
    ) -> None:
        mock_prediction.return_value = 105.01  # більше ніж на 5%
        result = cryptocurrency_action(100.0)
        self.assertEqual(result, "Buy more cryptocurrency")

    @patch("app.main.get_exchange_rate_prediction")
    def test_sell_all_when_prediction_low(
        self, mock_prediction: Mock
    ) -> None:
        mock_prediction.return_value = 94.99  # менше ніж на 5%
        result = cryptocurrency_action(100.0)
