...
@patch('app.main.get_exchange_rate_prediction')
def test_do_nothing_upper_boundary(mock_prediction) -> None:
    mock_prediction.return_value = 1.05
    assert cryptocurrency_action(1.0) == "Do nothing"

@patch('app.main.get_exchange_rate_prediction')
def test_do_nothing_lower_boundary(mock_prediction) -> None:
    mock_prediction.return_value = 0.95
    assert cryptocurrency_action(1.0) == "Do nothing"

@patch('app.main.get_exchange_rate_prediction')
def test_do_nothing_small_change(mock_prediction) -> None:
    mock_prediction.return_value = 1.02
    assert cryptocurrency_action(1.0) == "Do nothing"
